...
class CarInterface(CarInterfaceBase):
  ...
  def get_params(candidate, fingerprint=gen_empty_fingerprint(), has_relay=False, car_fw=[]):
    ...
    #else:
    ret.gasMaxBP = [0., 9., 55]
    ret.gasMaxV = [0.05, 0.05, 0.2]
    ret.longitudinalTuning.kpV = [0.325, 0.325, 0.325]  # braking tune from rav4h
    ret.longitudinalTuning.kiV = [0.15, 0.10]

    if candidate not in [CAR.PRIUS_2019, CAR.PRIUS, CAR.RAV4, CAR.OLD_CAR, CAR.RAV4H]: # These cars use LQR/INDI
    ...
    if candidate in [CAR.PRIUS, CAR.PRIUS_2019]:
      ...
      ret.longitudinalTuning.kiV = [0.02, 0.02]
